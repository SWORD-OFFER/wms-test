...
class InventoryQueryServiceTest {
    ...
    void queryInventory_关键词命中() {
        PageResult<InventoryResponse> result = inventoryService.queryInventory("蓝牙", null, 1, 100);
    ...
    void queryInventory_关键词未命中() {
        PageResult<InventoryResponse> result = inventoryService.queryInventory("不存在的商品XYZ", null, 1, 20);
        assertEquals(0, result.getTotal());
    }

    @Test
    void queryInventory_仓库筛选_仅返回该仓库数据() {
        PageResult<InventoryResponse> result = inventoryService.queryInventory(null, 1L, 1, 100);
        assertTrue(result.getTotal() > 0);
    ...
    void queryInventory_分页_总数一致且不超页大小() {
        PageResult<InventoryResponse> full = inventoryService.queryInventory(null, null, 1, 100);
        PageResult<InventoryResponse> paged = inventoryService.queryInventory(null, null, 1, 2);
